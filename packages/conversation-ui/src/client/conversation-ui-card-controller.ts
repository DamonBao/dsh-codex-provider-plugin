...
/** What the conversation-ui card renders. It remains visible while its Host RPC loads. */
export interface ConversationCardState {
...
export interface ConversationCardFace {
  hooks: {
    conversationUiCard: SnapshotStore<ConversationCardState>
  }
...
export class ConversationCardController {
  private readonly store = createSnapshotStore<ConversationCardState>(this.projection())
  private loaded: ConversationSettingsView | undefined
  ...
  constructor(private readonly api: ConversationSettingsApi) {}
  ...
  getSnapshot(): ConversationCardState {
  ...
  inject(): ConversationCardFace {
    return {
      hooks: { conversationUiCard: this.store },
      edit: (value) => {
        this.staged = value
        this.failed = false
        this.publish()
  ...
  private projection(): ConversationCardState {
    return {
      ...
      saving: this.saving,
      failed: this.failed,
      thinkAutoExpand: this.staged ?? this.loaded?.thinkAutoExpand ?? DEFAULT_CONVERSATION_SETTINGS.thinkAutoExpand,
