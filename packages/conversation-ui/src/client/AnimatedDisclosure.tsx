// AnimatedDisclosure: the harness DisclosureRow chrome (24px row, trailing
// chevron, whole-row click/Enter/Space toggle) with one deliberate
...
export function AnimatedDisclosure({
  ...
  return (
    <div className={css.disclosureRoot} data-open={open || undefined}>
      ...
      >
        <span className={cx(css.disclosureLeading, leadingClassName)} data-disclosure-icon>
          {icon}
        </span>
        <span className={cx(css.disclosureTitle, titleClassName)}>{title}</span>
        {!open && collapsedContent}
        <span
          className={cx(css.disclosureTrailing, chevronClassName)}
